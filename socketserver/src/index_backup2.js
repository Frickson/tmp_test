...
lcIO.on('connection', (socket) => {
	...
	let ad = "lobby";//getActiveAdmin(lcIO) //auto assign
    ...
    socket.on('admin_join_room', (admindata) => {
        ...
        //socket.join(admindata.roomId, () => {
		socket.join("lobby", () => { //forces all admin to join lobby
    ...
    socket.on('admin_send_client_msg', (adminData) => {
		...
		socketClientListUpdate(lcIO, "lobby")//update list for all users
