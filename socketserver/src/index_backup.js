    ...
    try{
        ...
        if(allsocketsinfo) {
            ...
            if(socket) {
				...
				console.log("roomname:"+roomname+" info:"JSON.stringify(clientsInfo));
...
lcIO.on('connection', (socket) => {
	...
	let ad = getActiveAdmin(lcIO)
	console.log(ad)
	emitMsg(socket,'connect_avail', ad);
    ...
    socket.on('client_join_room', (clientData) => {
        socket.join(clientData.roomId, () => {
            ...
            // get the rooms info in this socket
            let rooms = Object.keys(socket.rooms)
			console.log(clientData.username+" joined room"+rooms[1]);
            socket.sessionData = {
                room: rooms[1], // store the room name in the socket session for this client
                isClientMah: true, // this socket is a client
    ...
    socket.on('admin_send_client_msg', (adminData) => {
		//get socket and LITERALLY change its
		let clientSocket = lcIO.sockets[adminData.clientSocketId];
		//set your room to be mine
		if(clientSocket.sessionData.room !== socket.sessionData.room){
			clientSocket.leave(clientSocket.sessionData.room);
			clientSocket.join(socket.sessionData.room);
			clientSocket.sessionData.room = socket.sessionData.room;
			for(var i in Object.keys(lcIO.adapter.rooms)){
					console.log('refreshing');
					socketClientListUpdate(lcIO, Object.keys(lcIO.adapter.rooms)[i])
			}
		}
    ...
    socket.on('client_send_admin_msg', (data) => {
		...
		for(var i in sockets){
			...
			if(sessionData&&!sessionData.isClientMah){
				...
				console.log("checking sending: "+sessionData.username +" "+ data.adminUsername)
